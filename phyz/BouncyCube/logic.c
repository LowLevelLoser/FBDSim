...
void RunGame(game_t *game){
    ...
    if(IsMouseButtonDown(MOUSE_BUTTON_LEFT)){
        game->block.position.x = mouse_position.x;
        game->block.position.y = mouse_position.y;
        game->block.rect.x = game->block.position.x - game->block.size.x/2;
        game->block.rect.y = game->block.position.y - game->block.size.y/2;
        if (frame_time >= .01){
            game->block.velocity.x = (game->block.position.x - game->block.position_buffer.x)/(frame_time*120);
            game->block.velocity.y = (game->block.position.y - game->block.position_buffer.y)/(frame_time*120);
            game->block.position_buffer.x = game->block.position.x;
            game->block.position_buffer.y = game->block.position.y;
            frame_time = 0.0001;
        }
        do_the_thing = false;
    }
